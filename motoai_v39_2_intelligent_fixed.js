/* motoai_v39_2_intelligent_fixed.js
   ✅ UI/UX: Premium iOS (Glassmorphism), safe-area, iOS keyboard fix
   ✅ SECURITY: escapeHtml chống XSS, không markdown, không link
   ✅ NLU: Scoring Intent, Entities (Model, Duration, Area, Name)
   ✅ DIALOG: Multi-step state (ASK_MODEL / ASK_DURATION) hoạt động thực sự
   ✅ SEARCH: BM25 + Extractive QA + Auto-Price Learn + boost /banggia, /thutuc
*/
(function(){
  ...
  function searchIndex(query, k=3){
    ...
    const avgdl= totalLen/Math.max(1,docs.length); const N=docs.length;

    const scored = docs.map(d=>{
      const qToks=new Set(tk(query)); const map=tf.get(d.id)||new Map();
      let s=0;
      qToks.forEach(t=>{
        const f = map.get(t)||0; if(!f) return;
        const c = df.get(t)||1;
        const idf = Math.log(1 + (N - c + 0.5)/(c + 0.5));
        const dl = tk(d.text).length||1;
        s += idf*(f*(k1+1))/(f + k1*(1 - b + b*(dl/avgdl)));
      });
      s += scoreDocMeta(d.meta, query);
