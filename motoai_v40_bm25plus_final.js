/* motoai_v40_bm25plus_final.js
   ✅ BASE: MotoAI v39 (UI Premium, NLU, Dialog, Auto-Price)
   ✅ SEARCH ENGINE: BM25+ (Adaptive Delta, Caching, Stopwords)
   ✅ CRAWLER: Last-Modified Header Support, Smart Freshness
   ✅ TUNING: Boost Capping, Phrase Length Check
*/
(function(){
  ...
  function phraseBoost(text, query) {
    ...
    // 🔥 Tuned: match nguyên câu, cần q đủ dài
  ...
  // 4. Main Search Function (BM25+ Logic Fixed & Tuned)
  function searchIndex(query, k = 3) {
    const cache = loadLearn();
    const docs = [];

    // Pre-process docs & tokenization
    ...
    const scored = docs.map(d => {
      ...
      qToks.forEach(term => {
        ...
        if (!c) return;        
        if (f === 0) return; // Fix: Skip if term not in doc
  ...
  // Legacy fetch for simple XML check
  ...
  async function pullPages(urls, stats){
    ...
    for(const u of urls.slice(0, CFG.maxPagesPerDomain)){
      // 🔥 Updated to use fetchTextWithMeta
      ...
      out.push({url:u, title, text:desc, ts:pageTs});
