/* motoai_v39_1_premium_ios_patched.js
   ✅ UPDATE: UI/UX Premium (Glassmorphism), Animation mượt
   ✅ FIX: XSS (escapeHtml), iOS Listener Doubling, Typing Duplication
   ✅ LOGIC: BM25 + Extractive QA + Auto-Price Learn
*/
(function(){
  ...
  // [PATCH 1] Helper Escape HTML để chống XSS
  ...
  #mta-header{
    ...
    border-bottom: 1px solid rgba(0,0,0,0.05); position: absolute; top:0; left:0; right:0; z-index: 10;
  }
  ...
  #mta-in{ flex:1; height:var(--m-in-h); border:1px solid #d1d1d6; border-radius:22px; padding:0 16px; font-size:var(--m-in-fs); background:#fff; color:var(--m-text); outline:none; -webkit-appearance:none; transition: border-color 0.2s; }
  ...
  // [PATCH 1] addMsg mới sử dụng escapeHtml
  function addMsg(role, text){
    ...
    // Escape an toàn rồi mới cho xuống HTML, thay newline bằng <br>
  ...
  // [PATCH 2] Cờ chặn gắn listener trùng lặp
  let vvBound = false; 

  // [PATCH 3] showTyping kiểm tra trùng lặp
  function showTyping(){
    const body = $("#mta-body"); if(!body) return;
    if($("#mta-typing")) return; // Đã có thì thôi
  ...
  // [PATCH 2] Logic gắn listener VisualViewport an toàn
  function adjustForIOS(){
    if(!window.visualViewport) return;
    if(vvBound) return; // Đã gắn rồi thì không gắn lại
